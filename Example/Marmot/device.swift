...
class device: NSObject {
  ...
  ///
  /// - Returns: SSID 信息
  @objc func ssid() -> [[String: Any]]? {
    guard let interfaceNames = CNCopySupportedInterfaces() as? [String] else { return nil }
    return interfaceNames.compactMap { (name) -> [String: Any]? in
  ...
  @objc func taptic(params: [String: Any]) {
    
    let level: Int = params["value"] as! Int
    if #available(iOS 10.0, *), supportTaptic(), let style = UIImpactFeedbackGenerator.FeedbackStyle(rawValue: level) {
  ...
  @objc func wlanAddress() -> [String: Any]? {
    return ["value": getWlanAddress() ?? ""]
  }
  
  @objc func networkType() {
